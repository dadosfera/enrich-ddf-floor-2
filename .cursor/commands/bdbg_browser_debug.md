...
- When visual confirmation or advanced JS is required:
  - Use `puppeteer_screenshot(name=".tmp/<slug>", selector="<optional selector>")` to capture the broken state (always use `.tmp/` for temporary debug artifacts).
