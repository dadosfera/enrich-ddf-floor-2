...
BL_TARGET="${PLANS_BASE}/backlog/${OBJ_SLUG}_next_actions_${DATE}.md"

# Optional prioritized plan target (created only if flag set)
PR_TARGET="${PLANS_BASE}/prioritized/${OBJ_SLUG}_plan.md"
...
```bash
gtimeout 5 mkdir -p "$PLANS_BASE/backlog" "$PLANS_BASE/prioritized" "$PLANS_BASE/active" "$PLANS_BASE/finished"
```
...
PLAN_GLOB="${PLANS_BASE}/active/*${OBJ_SLUG}*_plan.md"
...
if [ -n "$ACTIVE_PLANS" ]; then
  ...
  if [ "$ALL_DONE" = "true" ]; then
    ...
    mkdir -p "$PLANS_BASE/finished"
    for PLAN in $ACTIVE_PLANS; do
      BASENAME=$(basename "$PLAN")
      FINISHED_PLAN="$PLANS_BASE/finished/$BASENAME"
...
- Do not create or update active plans (e.g., `docs/plans/active/` or `_dev/docs/plans/active/`) from this `/arch_archive` flow; the only allowed interaction is **moving fully completed plans out to finished/ during archival**. Active-plan alignment and editing belong to the mid-journey command (`/pfac_plan_from_active_tasks_conversation`).
- **Duplicate handling**: If a plan with the same name already exists in finished/, the AI model executing this command must analyze both files and merge them intelligently. The merge should combine unique content, preserve the most recent updates, consolidate completed tasks, and ensure the merged plan maintains `Status: finished`/`completed`/`done`. After successful merge, remove the plan from `/active/`.
...
- `- [ ]` / `- [x]` are recommended for task lists inside plans but not required. `/arch_archive` and related automation rely on each plan's canonical Status field to determine completion; missing checkboxes must not prevent archival when Status is `finished`/`completed`/`done`.

See also

- `standards/documentation/planning_docs_ci_standards.md` — global planning/docs/CI rules, including Status and checkbox expectations.
- `templates/plan_management_system.md` — detailed plan lifecycle and completion rules that `/arch_archive` follows.
