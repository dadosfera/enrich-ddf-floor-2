...
Use `/ddgn_deep_diagnostics` when:

1. Surface-level investigation (`/rcdg_root_cause_diag`) isn't providing enough insight
...
- The problem is obvious and well-understood (use `/rcdg_root_cause_diag` instead)
- You have direct access to already-detailed logs (use `/rcdg_root_cause_diag`)
...
**Pros/Cons**: [See rcdg_root_cause_diag format]
...
Based on enhanced data, create clear root cause analysis and solution options (see `/rcdg_root_cause_diag` format).
...
```
/ddgn_deep_diagnostics [completes]
↓
...
```
/ddgn_deep_diagnostics [completes with partial findings]
↓
...
```
/ddgn_deep_diagnostics [completes with clear diagnosis]
↓
...
| ------------------------------ | ---------------------- | ------------------------- | ---------------------------------------------- |
| `/rcdg_root_cause_diag`        | Quick diagnosis        | No                        | Analysis + proposal                            |
| **`/ddgn_deep_diagnostics`** | **Deep understanding** | **Yes (instrumentation)** | **Instrumentation + rich analysis + proposal** |
...
- `/rcdg_root_cause_diag` - Quick diagnosis without instrumentation
