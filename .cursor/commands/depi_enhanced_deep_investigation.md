...
Use `/depi_enhanced_deep_investigation` when:

1. Surface-level investigation (`/rciv_investigate_root_cause`, formerly `/invr_investigate_root_cause`) isn't providing enough insight
...
- The problem is obvious and well-understood (use `/rciv_investigate_root_cause` instead)
- You have direct access to already-detailed logs (use `/rciv_investigate_root_cause`)
...
**Pros/Cons**: [See rciv_investigate_root_cause format]
...
Based on enhanced data, create clear root cause analysis and solution options (see `/rciv_investigate_root_cause` format).
...
```
/depi_enhanced_deep_investigation [completes]
↓
/xect_execute_plan [execute the identified fix]
```

### If More Investigation Needed

```
/depi_enhanced_deep_investigation [completes with partial findings]
↓
 /help_plan_escalation_for_help [request help from higher-model]
```

### If Creating a Formal Plan

```
/depi_enhanced_deep_investigation [completes with clear diagnosis]
↓
...
| --------------------------------------- | ---------------------- | ------------------------- | ---------------------------------------------- |
| `/rciv_investigate_root_cause`          | Quick diagnosis        | No                        | Analysis + proposal                            |
| **`/depi_enhanced_deep_investigation`** | **Deep understanding** | **Yes (instrumentation)** | **Instrumentation + rich analysis + proposal** |
...
- `/rciv_investigate_root_cause` - Quick diagnosis without instrumentation
...
- `/help_plan_escalation_for_help` - Request help if investigation is inconclusive
