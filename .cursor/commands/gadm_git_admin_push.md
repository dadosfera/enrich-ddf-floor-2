...
<!-- COMMAND_TYPE: ga_git_admin_push -->
...
7. **AI executes**: Sync remotes and fetch all branches

```bash
gtimeout 10 git fetch --all --prune
```

8. **AI executes**: Discover available branches (local and remote)

```bash
gtimeout 10 git branch -a | grep -E "(main|master)" || true
```

8a. **AI executes**: Verify current branch and detect if it exists locally or remotely
...
```bash
# Check if current branch exists locally
...
```bash
CURRENT_BRANCH=$(gtimeout 5 git branch --show-current)
echo "About to push branch: $CURRENT_BRANCH"
```

```bash
# Determine branch reference: prefer local if exists, otherwise use remote
if gtimeout 10 git show-ref --verify --quiet refs/heads/"$CURRENT_BRANCH" 2>/dev/null; then
  BRANCH_REF="$CURRENT_BRANCH"
  echo "Using local branch: $BRANCH_REF"
else
  BRANCH_REF="origin/$CURRENT_BRANCH"
  echo "Using remote branch: $BRANCH_REF"
fi
```
...
```bash
# Push to the detected branch reference
gtimeout 15 git push origin "$CURRENT_BRANCH"
```
