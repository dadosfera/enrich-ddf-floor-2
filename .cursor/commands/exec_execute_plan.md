...
**Local Reference**: `commands/exec_execute_plan.md`
**Git URL Reference**: `https://github.com/dadosfera/docs-fera/blob/main/commands/exec_execute_plan.md`
...
- After expanding a conversation-related plan with `/expp_expand_plan`
...
- When you need to review conversation first (use `/revw_review`)
...
# - A plan that was expanded with /expp_expand_plan
...
/expp_expand_plan _dev/docs/plans/active/QW_2h_HIGH_implement_feature.md
...
3. `/expp_expand_plan` → Enrich plan with details and research
4. **`/exec_execute_plan`** → Execute the comprehensive plan
...
- **`/expp_expand_plan`**: Add depth, validation, and research to existing plan
- **`/exec_execute_plan`**: Execute the conversation-related plan and show its absolute path
...
- **`/expp_expand_plan`**: Enriches plans before this command executes them
...
bash tests/commands/test_exec_execute_plan.sh
