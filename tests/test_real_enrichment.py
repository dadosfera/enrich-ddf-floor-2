...
def print_result(title: str, data: dict, highlight_real: bool = False):
    ...
    if highlight_real and "mock" not in str(data.get("data_sources", [])):
        ...
        print(
            "🚨 MOCK DATA (Configure API keys for real data)"
...
def check_api_keys():
    ...
    for (
        key,
        value,
    ) in keys.items():  # TODO: Review loop variable naming (PLW2901)  # TODO: Review loop variable naming (PLW2901)
    ...
    if not configured_keys:
        ...
        print("  - GitHub: https://github.com/settings/tokens (5,000 requests/hour)")
...
async def benchmark_enrichment_speed():
    ...
    for i, email in enumerate(test_emails):
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ {email}: {request_duration:.2f}s - Error {response.status_code}"
                )
    ...
    average_duration = (
        sum(r["duration"] for r in results if r.get("success"))
        / max(  # TODO: Review loop variable naming (PLW2901)  # TODO: Review loop variable naming (PLW2901)
