...
def test_hunter_io(api_key: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return False, "❌ Hunter.io: Invalid API key"
        else:
            return False, f"❌ Hunter.io: HTTP {response.status_code}"
    except Exception as e:
        return False, f"❌ Hunter.io: {e!s}"
...
def test_apollo(api_key: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return False, "❌ Apollo.io: Invalid API key"
        else:
            return False, f"❌ Apollo.io: HTTP {response.status_code}"
    except Exception as e:
        return False, f"❌ Apollo.io: {e!s}"
...
def test_wiza(api_key: str) -> Tuple[bool, str]:
    ...
    try:
        response = requests.get(url, headers=headers, timeout=10)
        if response.status_code == 200:
            return True, "✅ Wiza: API connection successful"
        elif response.status_code == 401:
            return False, "❌ Wiza: Invalid API key"
            ...
            if response2.status_code == 200:
                return (
                    True,
                    "✅ Wiza: API connection successful (alternative endpoint)",
                )
            else:
                return False, "❌ Wiza: API endpoints not accessible (404)"
        else:
            return False, f"❌ Wiza: HTTP {response.status_code}"
    except Exception as e:
        return False, f"❌ Wiza: {e!s}"
...
def test_surfe(api_key: str) -> Tuple[bool, str]:
    ...
    try:
        response = requests.get(url, headers=headers, timeout=10)
        if response.status_code == 200:
            return True, "✅ Surfe: API connection successful"
        elif response.status_code == 401:
            return False, "❌ Surfe: Invalid API key"
        elif response.status_code == 404:
            # Try alternative authentication method
            headers_alt = {"Authorization": f"Bearer {api_key}"}
            response2 = requests.get(url, headers=headers_alt, timeout=10)
            if response2.status_code == 200:
                return True, "✅ Surfe: API connection successful (Bearer auth)"
            else:
                return (
                    False,
                    "❌ Surfe: API endpoints not accessible - may need documentation review",
                )
        else:
            return False, f"❌ Surfe: HTTP {response.status_code}"
    except Exception as e:
        return False, f"❌ Surfe: {e!s}"
...
def test_coresignal(api_key: str) -> Tuple[bool, str]:
    ...
    # Simple test query
    payload = {"limit": 1, "title": "CEO"}

    try:
        response = requests.post(url, headers=headers, json=payload, timeout=15)
        if response.status_code == 200:
            return True, "✅ Coresignal: API connection successful"
        elif response.status_code == 401:
            return False, "❌ Coresignal: Invalid API key"
        elif response.status_code == 403:
            return False, "❌ Coresignal: API key lacks permissions"
        elif response.status_code == 404:
            ...
            if response2.status_code == 200:
                return (
                    True,
                    "✅ Coresignal: API connection successful (account endpoint)",
                )
            else:
                return False, "❌ Coresignal: API endpoints not accessible"
        else:
            return False, f"❌ Coresignal: HTTP {response.status_code}"
    except Exception as e:
        return False, f"❌ Coresignal: {e!s}"
...
def test_bigdata_corp(api_key: str, secret: str) -> Tuple[bool, str]:
    ...
    try:
        response = requests.get(url, headers=headers, timeout=10)
        if response.status_code == 200:
            return True, "✅ BigData Corp: API connection successful"
        elif response.status_code == 401:
            return False, "❌ BigData Corp: Invalid credentials"
        else:
            return False, f"❌ BigData Corp: HTTP {response.status_code}"
    except Exception as e:
        return False, f"❌ BigData Corp: {e!s}"
...
def main():
    ...
    if "BIGDATA_CORP_API_KEY" in api_keys and "BIGDATA_CORP_SECRET" in api_keys:
        enhanced_tests.append(
            (
                "BigData Corp (🇧🇷)",
                None,
                lambda _: test_bigdata_corp(
                    api_keys["BIGDATA_CORP_API_KEY"], api_keys["BIGDATA_CORP_SECRET"]
                ),
            )
        )
    ...
    for service, success, message in results:
        ...
        if success:
            ...
            "placeholder" in message.lower() or "needs configuration" in message.lower()
        ):
