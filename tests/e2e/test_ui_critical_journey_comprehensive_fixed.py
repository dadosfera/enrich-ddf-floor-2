...
class FixedComprehensiveUITest:
    ...
    async def api_request_with_retry(self, url, method="GET", **kwargs):
        ...
        for attempt in range(3):  # Reduced from 10 to 3
            try:
                response = requests.request(method, url, timeout=10, **kwargs)  # Reduced from 20s to 10s
                return response
    ...
    async def test_api_data_verification(self) -> bool:
        ...
        try:
            # Verify companies
            companies_response = await self.api_request_with_retry(f"{API_BASE_URL}/companies")
            ...
            # Verify contacts
            contacts_response = await self.api_request_with_retry(f"{API_BASE_URL}/contacts")
            ...
            # Verify products
            products_response = await self.api_request_with_retry(f"{API_BASE_URL}/products")
