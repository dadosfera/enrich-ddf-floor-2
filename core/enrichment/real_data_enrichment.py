...
class RealDataEnrichmentEngine:
    ...
    async def enrich_person_real(self, person_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'clearbit' in self.services and self.quota_manager.can_make_request('clearbit') and email:
            try:
        ...
        if 'hunter' in self.services and self.quota_manager.can_make_request('hunter') and email:
            try:
    ...
    def _merge_clearbit_data(self, enriched_data: Dict, clearbit_result: Dict):
        ...
        if person:
            enriched_data['full_name'] = person.get('full_name') or enriched_data['full_name']
        ...
        if company:
            ...
            enriched_data['professional']['company_industry'] = company.get('category', {}).get('industry')
    
    def _merge_hunter_data(self, enriched_data: Dict, hunter_result: Dict):
        """Merge Hunter.io API response into enriched data."""
        enriched_data['contact']['email_verified'] = hunter_result.get('result') == 'deliverable'
        enriched_data['contact']['email_confidence'] = hunter_result.get('score', 0)
        enriched_data['contact']['email_disposable'] = hunter_result.get('disposable', False)
    ...
    def _calculate_enrichment_score(self, data: Dict) -> int:
        ...
        key_fields = [
            'full_name', 'email', 
            'professional.current_title', 'professional.current_company',
    ...
    def _generate_enhanced_mock_data(self, person_data: Dict) -> Dict:
        ...
        email = person_data.get('email', f'{first_name.lower()}.{last_name.lower()}@example.com')
    ...
    async def enrich_company_real(self, company_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'clearbit' in self.services and self.quota_manager.can_make_request('clearbit') and domain:
            try:
                ...
                if result.get('success'):
                    ...
                    self.quota_manager.record_request('clearbit')
                    enriched_data['data_sources'].append('clearbit')
                    logger.info(f"✅ Clearbit company enrichment successful for {domain}")
            except Exception as e:
                logger.error(f"Clearbit company error: {e}")
        
        enriched_data['enrichment_score'] = self._calculate_company_enrichment_score(enriched_data)
