# /rerr_recurrent_errors
<!-- COMMAND_ID: 039 -->
<!-- COMMAND_VERSION: 1.0.0 -->
<!-- COMMAND_TYPE: re_recurrent_errors -->
...
- https://github.com/dadosfera/docs-fera/blob/main/recurrent_errors/README.md
...
5. For preventative context and behavior enforcement, explicitly update:
   - Rules files: `.cursor/rules/`, `.dadosfera/rules/`, `.cursorrules`, or `.dadosferarules` to enforce the fix or pattern.
   - Local documentation: `AGENTS.md` and `README.md` in the relevant folders to warn about the error and document the correct procedure.
...
```yaml
- path: "${REC_DIR}/${DATE}_${ERR_SLUG}.md"
  status: "recurrent" # or "one_off"
  description: "Short, human-readable title"
```

8. Add backlinks in affected files (manual edit)

- In each affected file (scripts, docs, workflows, config), add a small “Related” or “See also” section with a link back to the new entry.
- Use the path stored in `$REC_DIR` (either `docs/recurrent_errors/` or `_dev/docs/recurrent_errors/`).

```markdown
Related: ${REC_DIR}/${DATE}\_${ERR_SLUG}.md
```
