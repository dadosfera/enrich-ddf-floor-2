# /rcdg_root_cause_diag
<!-- COMMAND_ID: 009 -->
<!-- COMMAND_VERSION: 1.0.0 -->
<!-- COMMAND_TYPE: in_root_cause_diag -->
...
**Local Reference**: `commands/rcdg_root_cause_diag.md`
**Git URL Reference**: `https://github.com/dadosfera/docs-fera/blob/main/commands/rcdg_root_cause_diag.md`
...
- `commands/ddgn_deep_diagnostics.md`

## When to use this command

Use `/rcdg_root_cause_diag` when:
...
- You already know the fix and just need to implement it (use `/xect_execute_plan` instead)
...
- Surface-level investigation isn't enough (use `/ddgn_deep_diagnostics` instead)
...
4. **Further investigation**: If more information is needed, use `/ddgn_deep_diagnostics` to add instrumentation
...
Status:
  ...
  - Option 2: Execute directly with /xect_execute_plan
  - Option 3: Share for team feedback
  - Option 4: Deep investigation with /ddgn_deep_diagnostics
...
```
# After /rcdg_root_cause_diag completes:
...
```
# After /rcdg_root_cause_diag completes (but didn't provide enough insight):
/ddgn_deep_diagnostics
```
...
```
# After /rcdg_root_cause_diag completes:
# Share the diagnosis file with the team, then use /help_plan_escalation_for_help if needed
```

## Related Commands

- `/xect_execute_plan` - Execute a solution directly after investigation
- `/pfac_plan_from_active_tasks_conversation` - Create a formal plan from a diagnosis
- `/ddgn_deep_diagnostics` - Add instrumentation for deeper investigation
