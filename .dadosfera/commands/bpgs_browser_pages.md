...
**Required for**: UI/web repository PRs that affect front-end behavior (see `standards/documentation/planning_docs_ci_standards.md`).

Backlinks:

- `guides/tools/browser/browser_pages_visual_journey.md` — detailed guide on when and how to use this command
- `standards/documentation/planning_docs_ci_standards.md` — PR checklist requirement
...
5. **For each navigation target** (starting from home each time):
   ...
   - **Capture visuals**:
     - **Storage Rules**:
       - **Temporary/Ad-hoc**: Use `.tmp/<page-slug>.png` (local repo temp folder). Do NOT use system temp folders.
       - **CI/CD / Tests**: Use `tests/screenshots/<page-slug>.png` (or appropriate test subfolder).
     - With Cursor Browser Extension: `browser_take_screenshot(type="png", fullPage=true, filename=".tmp/<page-slug>.png")`
     - With Puppeteer: `puppeteer_screenshot(name=".tmp/<page-slug>", selector="body")`
...
- For UI/web repos, this command is **required** before merging PRs that affect front-end behavior (see `standards/documentation/planning_docs_ci_standards.md`).
