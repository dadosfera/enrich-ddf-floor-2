...
def enrich_person_data(person: Dict[str, Any]):
    ...
    try:
        response = requests.post(
            f"{API_BASE_URL}/api/v1/enrich/person",
            headers=HEADERS,
            json=payload
        )
        ...
        if response.status_code == 200:
            ...
            print(f"   Location: {enriched['location']['city']}, {enriched['location']['state']}")
            print(f"   Skills: {', '.join(enriched['skills'][:3])}...")
            print(f"   Enrichment Score: {enriched_data['data']['enrichment_score']}/100")
...
def enrich_company_data(company: Dict[str, Any]):
    ...
    try:
        response = requests.post(
            f"{API_BASE_URL}/api/v1/enrich/company",
            headers=HEADERS,
            json=payload
        )
        ...
        if response.status_code == 200:
            ...
            print(f"   Location: {enriched['location']['headquarters']['city']}, {enriched['location']['headquarters']['state']}")
...
def test_pdl_integration():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   Current Role: {result['data']['current_role']['title']} at {result['data']['current_role']['company']}")
...
def create_new_records():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Created new person: {person_result['data']['first_name']} {person_result['data']['last_name']}")
